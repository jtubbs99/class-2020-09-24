...
```{r}
total_enroll <- left_join(sep_new, sep_old, 
                           by = c("id", "title", "name", "department"), 
                          suffix = c("_new", "_old")) %>% 
  mutate(change_enroll = (u_grad_new - u_grad_old))

most_inc <- total_enroll %>% 
  select(id, name, u_grad_new, change_enroll, u_grad_old) %>% 
  arrange(desc(change_enroll)) %>% 
  slice_head(n = 5)

most_dec <- total_enroll %>% 
  select(id, name, u_grad_new, change_enroll, u_grad_old) %>% 
  arrange(change_enroll) %>% 
  slice_head(n = 5)

both_info <- full_join(most_inc, most_dec, 
                       by = c("id", "name", "change_enroll", "u_grad_new", "u_grad_old"))
```

```{r}
most_inc %>% 
  gt() %>% 
  tab_header(title = "Largest Changes in Undergrad Enrollment", 
             subtitle = "5 Most Increased From 9/1 to 9/22")
...
most_dec %>% 
  gt() %>% 
  tab_header(title = "Largest Changes in Undergrad Enrollment", 
             subtitle = "5 Most Decreased From 9/1 to 9/22")
...
```{r}
ggplot(both_info, aes(name, change_enroll)) +
  geom_col() +
  labs(title = "Largest Changes in Undergrad Enrollment", 
